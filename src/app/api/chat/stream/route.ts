  ...
  try {
    ...
    if (convError) {
      ...
      if (convError.code === 'PGRST116') {
        console.log('Conversation not found, creating new one:', conversation_id)
    ...
    let marketContext = ''
    let directMarketResponse: { response: string; metadata: Record<string, unknown> } | null = null
    ...
    if (includeMarketData) {
      try {
        ...
        if (marketResponse.response && marketResponse.metadata.model === 'intelligent-market') {
          directMarketResponse = marketResponse
        } else if (marketResponse.metadata.symbols && marketResponse.metadata.symbols.length > 0) {
          ...
          try {
            ...
            const { error: aiMessageError } = await supabase
              ...
              .insert({
                ...
                role: 'assistant',
                content: directMarketResponse.response,
                metadata: directMarketResponse.metadata ? JSON.parse(JSON.stringify(directMarketResponse.metadata)) : null
              })
            ...
            controller.enqueue(
              encoder.encode(`data: ${JSON.stringify({
                type: 'error',
                error: error instanceof Error ? error.message : 'Erro na análise de mercado'
              })}\n\n`)
            )
          } finally {
            controller.close()
            ...
            if (chunk.done) {
              ...
              const { error: aiMessageError } = await supabase
                ...
                .insert({
                  ...
                  metadata: chunk.metadata ? JSON.parse(JSON.stringify(chunk.metadata)) : null
                })
