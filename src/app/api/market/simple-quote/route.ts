...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log('Alpha Vantage failed, trying Yahoo Finance...', alphaError instanceof Error ? alphaError.message : 'Unknown error')
    }
    ...
    try {
      ...
      console.log('Yahoo Finance also failed', yahooError instanceof Error ? yahooError.message : 'Unknown error')
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        change: -0.50 + Math.random() * 2,
        changePercent: (Math.random() * 6 - 3).toFixed(2) + '%',
        volume: Math.floor(Math.random() * 10000000),
        high: 26.00 + Math.random() * 5,
        low: 24.00 + Math.random() * 3,
        open: 25.00 + Math.random() * 2,
