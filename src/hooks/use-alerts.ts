...
import type { Alert, CreateAlert, UpdateAlert, AlertType, ConditionType } from '@/lib/types/alerts'
...
export function useAlerts(): UseAlertsReturn {
  ...
  // Create new alert
  const createAlert = useCallback(async (config: CreateAlert): Promise<Alert | null> => {
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) {
        setError('User not authenticated')
  ...
  // Update alert
  const updateAlert = useCallback(async (id: string, updates: UpdateAlert): Promise<Alert | null> => {
    try {
      const { data: { session } } = await supabase.auth.getSession()
  ...
  const getTriggeredAlerts = useCallback(() => {
    return alerts.filter(alert => alert.triggered_at !== null && alert.triggered_at !== undefined)
  }, [alerts])

  // Check alerts against current market data
  const checkAlerts = useCallback(async (symbol?: string) => {
    try {
      ...
      const symbolGroups = alertsToCheck.reduce((groups, alert) => {
      ...
      // Check each symbol
      for (const [checkSymbol, symbolAlerts] of Object.entries(symbolGroups)) {
        try {
          ...
          for (const alert of symbolAlerts) {
            const currentValue = alert.alert_type === 'price' 
              ? quote.price 
            ...
            switch (alert.condition_type) {
              ...
              case 'below':
                ...
                break
              case 'cross_above':
                shouldTrigger = (alert.current_value || 0) <= alert.target_value && currentValue > alert.target_value
                break
              case 'cross_below':
                shouldTrigger = (alert.current_value || 0) >= alert.target_value && currentValue < alert.target_value
                break
