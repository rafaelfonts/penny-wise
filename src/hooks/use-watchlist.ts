...
export function useWatchlist(): UseWatchlistReturn {
  ...
  const fetchWatchlist = useCallback(async () => {
    try {
      ...
      const transformedData: WatchlistItem[] = (data || []).map(item => ({
        ...
        alerts: typeof item.alerts === 'object' && item.alerts !== null 
  ...
  const addSymbol = useCallback(async (symbol: string): Promise<boolean> => {
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) {
        setError('User not authenticated')
      ...
      const quoteResponse = await marketDataService.getQuote(symbol.toUpperCase())
      ...
      const newItem: WatchlistItem = {
        ...
        alerts: typeof data.alerts === 'object' && data.alerts !== null 
  ...
  const removeSymbol = useCallback(async (symbol: string): Promise<boolean> => {
    try {
      const { data: { session } } = await supabase.auth.getSession()
  ...
  // Update position
  const updatePosition = useCallback(async (symbol: string, position: number): Promise<boolean> => {
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) return false
  ...
  const refreshAll = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setWatchlist(prev => prev.map(item => ({
          ...item,
          quote: quotesMap.get(item.symbol) || item.quote,
          isLoading: false
        })))
      } else {
  ...
  // Reorder watchlist
  const reorderWatchlist = useCallback(async (startIndex: number, endIndex: number): Promise<boolean> => {
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) return false
      ...
      setWatchlist(reorderedItems.map((item, index) => ({
        ...item,
        position: index + 1
      })))
