...
export class OplabService {
  ...
  async authenticate(
    email: string,
    password: string
  ): Promise<OplabResponse<UserInfo>> {
  ...
  async searchInstruments(
    expr: string
  ): Promise<OplabResponse<InstrumentSearchResult[]>> {
    return this.makeRequest<InstrumentSearchResult[]>(
      `/v2/charts/instruments/search/${encodeURIComponent(expr)}`
    );
  }
  ...
  async getCurrentQuotes(
    ...
    return this.makeRequest<Record<string, unknown>>(
      `/market/quotes?symbols=${symbolsParam}`
    );
  }
  ...
  async getInstrument(
    ...
    return this.makeRequest<Record<string, unknown>>(
      `/market/instruments/${symbol}`
    );
  }

  // Listar séries de opções de um instrumento
  async getInstrumentOptionSeries(
    symbol: string
  ): Promise<OplabResponse<unknown[]>> {
    return this.makeRequest<unknown[]>(
      `/market/instruments/${symbol}/option-series`
    );
  }
  ...
  async createPortfolio(
    name: string,
    active: boolean = true
  ): Promise<OplabResponse<Portfolio>> {
    return this.makeRequest<Portfolio>('/domain/portfolios', {
      method: 'POST',
  ...
  async getOptions(underlyingSymbol: string): Promise<OplabResponse<Option[]>> {
    return this.makeRequest<Option[]>(
      `/market/options?underlying=${underlyingSymbol}`
    );
  }
  ...
  async getCoveredOptions(
    underlyingSymbol: string,
    strategy?: string
  ): Promise<OplabResponse<Option[]>> {
    const params = strategy ? `?strategy=${strategy}` : '';
    return this.makeRequest<Option[]>(
      `/market/options/${underlyingSymbol}/covered${params}`
    );
  }

  // Consultar Black-Scholes de uma opção
  async getOptionBlackScholes(
    optionSymbol: string
  ): Promise<OplabResponse<BlackScholesData>> {
    return this.makeRequest<BlackScholesData>(
      `/market/options/${optionSymbol}/black-scholes`
    );
  }
  ...
  async getTopVolumeOptions(): Promise<OplabResponse<Record<string, unknown>>> {
    return this.makeRequest<Record<string, unknown>>(
      '/market/rankings/options/volume'
    );
  }

  // Opções com maiores taxas de lucro
  async getHighestProfitOptions(): Promise<
    OplabResponse<Record<string, unknown>>
  > {
    return this.makeRequest<Record<string, unknown>>(
      '/market/rankings/options/profit'
    );
  }

  // Opções com maiores variações
  async getBiggestVariationOptions(): Promise<
    OplabResponse<Record<string, unknown>>
  > {
    return this.makeRequest<Record<string, unknown>>(
      '/market/rankings/options/variation'
    );
  }
  ...
  async getTrendingOptions(
    ...
    return this.makeRequest<Record<string, unknown>>(
      `/market/rankings/options/trends${params}`
    );
  }

  // Opções ordenadas pela correlação com IBOV
  async getIbovCorrelationOptions(): Promise<
    OplabResponse<Record<string, unknown>>
  > {
    return this.makeRequest<Record<string, unknown>>(
      '/market/rankings/options/ibov-correlation'
    );
  }

  // Companhias ordenadas por atributo fundamentalista
  async getFundamentalistCompanies(
    attribute: string
  ): Promise<OplabResponse<Record<string, unknown>>> {
    return this.makeRequest<Record<string, unknown>>(
      `/market/rankings/companies/fundamentals?attribute=${attribute}`
    );
  }
  ...
  async getOplabScoreStocks(): Promise<OplabResponse<Record<string, unknown>>> {
    return this.makeRequest<Record<string, unknown>>(
      '/market/rankings/stocks/oplab-score'
    );
  }
  ...
  async getHistoricalData(
    ...
    return this.makeRequest<Record<string, unknown>>(
      `/market/historical/${symbol}${queryString ? `?${queryString}` : ''}`
    );
  }
  ...
  async getOptionsHistory(
    ...
    return this.makeRequest<Record<string, unknown>>(
      `/market/options/${underlyingSymbol}/history${params}`
    );
  }

  async healthCheck(): Promise<
    OplabResponse<{ status: string; timestamp: string }>
  > {
...
export const getOplabService = (): OplabService => {
  if (!instance) {
    throw new Error(
      'Oplab service not initialized. Call createOplabService first.'
    );
  }
  return instance;
};
