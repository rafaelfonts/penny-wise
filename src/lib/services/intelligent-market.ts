...
class IntelligentMarketService {
    ...
    try {
      ...
      if (classification.region === 'BR') {
        try {
          ...
          console.warn(`OpLab failed for ${symbol}, trying Alpha Vantage:`, oplabError)
          // Fallback para Alpha Vantage com símbolo .SA
          return await this.getAlphaVantageQuote(symbol + '.SA', classification)
        }
      }
   ...
   */
  private async getOplabQuote(symbol: string, classification: MarketClassification): Promise<IntelligentQuote | null> {
    await this.ensureOplabInitialized()
   ...
   */
  private async getAlphaVantageQuote(symbol: string, classification: MarketClassification): Promise<IntelligentQuote | null> {
    const apiKey = process.env.ALPHA_VANTAGE_API_KEY
    if (!apiKey) {
      throw new Error('Alpha Vantage not configured - missing ALPHA_VANTAGE_API_KEY')
    }
   ...
   */
  private buildErrorAnalysis(symbol: string, classification: MarketClassification): string {
    const suggestions = []
    
    if (classification.region === 'BR') {
      suggestions.push('💡 Verifique se o símbolo está correto (ex: PETR4, VALE3)')
      suggestions.push('💡 Empresas brasileiras terminam com dígito (3=ON, 4=PN, 11=Units)')
      ...
      suggestions.push('💡 Verifique se o símbolo está correto (ex: AAPL, MSFT)')
      suggestions.push('💡 Empresas americanas são 1-5 letras sem números')
      suggestions.push('📱 Recomendo verificar em: Yahoo Finance ou Google Finance')
    }
...
*Os dados de mercado serão restaurados em breve. Obrigado pela compreensão!*`
   ...
   */
  private buildIntelligentAnalysis(quote: IntelligentQuote, classification: MarketClassification): string {
    ...
    const flag = classification.region === 'BR' ? '🇧🇷' : classification.region === 'US' ? '🇺🇸' : '🌍'
...
*Fonte: ${quote.source} | ${classification.exchange} | Confiança: ${Math.round(classification.confidence * 100)}%*`
   ...
   */
  private generateInsights(quote: IntelligentQuote, classification: MarketClassification): string {
    const insights = []
    const changePercent = parseFloat(quote.changePercent.replace('%', ''))
    
    if (classification.region === 'BR') {
      if (changePercent > 5) {
        insights.push('🚀 Movimento forte para ação brasileira - acompanhe notícias setoriais')
      }
      insights.push('📰 Considere impacto do cenário político e econômico brasileiro')
      if (quote.volume > 1000000) {
        insights.push('📊 Volume elevado - interesse institucional significativo')
      }
    } else if (classification.region === 'US') {
      if (changePercent > 3) {
        insights.push('🇺🇸 Performance forte no mercado americano - verifique earnings')
      }
      insights.push('💰 Considere variação cambial USD/BRL para investidores brasileiros')
      if (quote.volume > 10000000) {
        insights.push('🔥 Volume muito alto - possível catalisador ou notícia relevante')
      }
    }
...
// Helper functions
export const getIntelligentQuote = (symbol: string) => intelligentMarket.getQuote(symbol)
export const analyzeIntelligentSymbol = (symbol: string) => intelligentMarket.analyzeSymbol(symbol) 