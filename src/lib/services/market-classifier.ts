...
class MarketClassifierService {
    ...
    return {
      ...
      exchange:
        region === 'BR'
          ? 'B3'
          : region === 'US'
            ? this.guessAmericanExchange(upperSymbol)
            : 'UNKNOWN',
  ...
  getSuggestions(classification: MarketClassification): string[] {
    ...
    if (classification.region === 'BR') {
      suggestions.push(
        '💡 Para empresas brasileiras, use símbolos como PETR4, VALE3, ITUB4'
      );
      suggestions.push(
        '💡 Adicione o dígito ao final (3 para ON, 4 para PN, 11 para Units)'
      );
    } else if (classification.region === 'US') {
      suggestions.push(
        '💡 Para empresas americanas, use símbolos como AAPL, MSFT, TSLA'
      );
      suggestions.push(
        '💡 Símbolos americanos são normalmente 1-5 letras sem números'
      );
    }

    if (classification.confidence < 0.7) {
      suggestions.push(
        '⚠️ Classificação incerta - verifique se o símbolo está correto'
      );
    }
...
// Helper functions
export const classifySymbol = (symbol: string) =>
  marketClassifier.classify(symbol);
export const classifySymbols = (symbols: string[]) =>
  marketClassifier.classifyMultiple(symbols);
export const getApiProvider = (symbol: string) =>
  marketClassifier.classify(symbol).apiProvider;
export const isAmericanStock = (symbol: string) =>
  marketClassifier.classify(symbol).region === 'US';
export const isBrazilianStock = (symbol: string) =>
  marketClassifier.classify(symbol).region === 'BR';
