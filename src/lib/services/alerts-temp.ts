...
export class AlertService {
  ...
  async checkAlerts(alertCheck: AlertCheck): Promise<AlertTrigger[]> {
    ...
    const userAlerts = this.mockAlerts.filter(a => 
      a.symbol === symbol && a.is_active
    )
  ...
  async deleteTriggeredAlerts(olderThanDays: number = 30): Promise<number> {
    ...
    const toDelete = this.mockAlerts.filter(a => 
      a.triggered_at && new Date(a.triggered_at) < cutoffDate
    )
