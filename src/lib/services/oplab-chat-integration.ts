...
class OplabChatIntegrationService {
  ...
  private readonly OPTIONS_KEYWORDS = [
    'opção', 'opções', 'call', 'put', 'strike', 'vencimento', 'exercício',
    'black-scholes', 'volatilidade', 'delta', 'gamma', 'theta', 'vega',
    'estratégia', 'covered call', 'protective put', 'straddle', 'strangle'
  ];

  private readonly FUNDAMENTALS_KEYWORDS = [
    'fundamentalista', 'p/l', 'roe', 'roce', 'dividend yield', 'endividamento',
    'liquidez', 'ebitda', 'lucro', 'receita', 'margem', 'patrimônio',
    'balanço', 'demonstrativo', 'indicador', 'fundamentals'
  ];

  private readonly RANKING_KEYWORDS = [
    'ranking', 'top', 'melhor', 'maior', 'menor', 'líder', 'destaque',
    'mais negociado', 'volume', 'variação', 'performance', 'score'
  ];

  private readonly CHART_KEYWORDS = [
    'gráfico', 'chart', 'histórico', 'preço', 'cotação', 'série temporal',
    'candlestick', 'ohlc', 'volume'
  ];

  private readonly SEARCH_KEYWORDS = [
    'buscar', 'procurar', 'encontrar', 'search', 'empresa', 'companhia'
  ];

  /**
   * Analyze chat message to determine required OpLab data
   */
  public analyzeMessage(message: string, detectedSymbols: string[]): ChatAnalysisRequest {
    const lowerMessage = message.toLowerCase();

    return {
      message,
      detectedSymbols,
      requiresOptions: this.OPTIONS_KEYWORDS.some(keyword => lowerMessage.includes(keyword)),
      requiresFundamentals: this.FUNDAMENTALS_KEYWORDS.some(keyword => lowerMessage.includes(keyword)),
      requiresRankings: this.RANKING_KEYWORDS.some(keyword => lowerMessage.includes(keyword)),
      requiresSearch: this.SEARCH_KEYWORDS.some(keyword => lowerMessage.includes(keyword)) || detectedSymbols.length === 0
    };
  }
   ...
   */
  public async getComprehensiveContext(request: ChatAnalysisRequest): Promise<OplabChatContext> {
   ...
   */
  private async getOptionsData(context: OplabChatContext, symbols: string[]): Promise<void> {
    try {
      const optionsPromises = symbols.slice(0, 3).map(async symbol => {
        try {
          const response = await fetch(`/api/market/oplab?action=options&symbol=${symbol}`);
          if (response.ok) {
            const result = await response.json();
  ...
  private async getFundamentalsData(context: OplabChatContext): Promise<void> {
    try {
      const promises = [
        ...
        fetch('/api/market/oplab?action=fundamentalist-companies&attribute=roe'),
        fetch('/api/market/oplab?action=fundamentalist-companies&attribute=dividend_yield')
      ];
   ...
   * Get search data
   */
  private async getSearchData(context: OplabChatContext, message: string): Promise<void> {
    try {
      ...
      const searchPromises = searchTerms.slice(0, 2).map(async term => {
        try {
          const response = await fetch(`/api/market/oplab?action=search-instruments&expr=${encodeURIComponent(term)}`);
          if (response.ok) {
            const result = await response.json();
            return result.success ? result.data : null;
          }
      ...
      const allInstruments = searchResults.filter(result => result !== null).flat();
   ...
   * Get companies data
   */
  private async getCompaniesData(context: OplabChatContext, symbols: string[]): Promise<void> {
    try {
      const companiesPromises = symbols.slice(0, 3).map(async symbol => {
        try {
          const response = await fetch(`/api/market/oplab?action=company&symbol=${symbol}`);
          if (response.ok) {
            const result = await response.json();
            return result.success ? result.data : null;
          }
  ...
  private extractSearchTerms(message: string): string[] {
    ...
    const potentialTerms = words.filter(word => {
      return /^[a-z]{3,}$/i.test(word) && 
             !['com', 'para', 'que', 'uma', 'dos', 'das', 'the', 'and', 'for', 'com', 'www'].includes(word);
    });
   ...
   */
  public async getBlackScholesData(optionSymbol: string): Promise<Record<string, unknown> | null> {
    try {
   ...
   */
  public async getChartData(symbol: string, resolution: string = '1D', from?: number, to?: number): Promise<unknown | null> {
    try {
  ...
  public getAvailableEndpoints(): Record<string, string> {
    return {
      ...
      'fundamentalist-companies': 'Empresas ordenadas por indicador fundamentalista',
