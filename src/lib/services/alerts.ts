...
export class AlertService {
  ...
  async checkAlerts(alertCheck: AlertCheck): Promise<AlertTrigger[]> {
    ...
    const { data, error } = await this.supabase.rpc('check_price_alerts', {
      symbol_param: symbol,
  ...
  async triggerAlert(alertId: string, currentValue: number): Promise<void> {
    ...
    const { error } = await this.supabase.rpc('trigger_alert', {
  ...
  subscribeToUserAlerts(userId: string, callback: (payload: Alert) => void) {
    ...
    console.warn('Real-time alerts subscription not implemented yet', {
      userId,
      callback,
    });
    return null;
  ...
  unsubscribeFromAlerts(
    subscription: ReturnType<typeof this.subscribeToUserAlerts>
  ) {
