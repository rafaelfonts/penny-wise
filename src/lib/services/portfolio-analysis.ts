...
export interface PortfolioAnalysisRequest {
  ...
  analysisType: 'performance' | 'risk' | 'diversification' | 'recommendations' | 'complete';
...
class PortfolioAnalysisService {
  ...
  async analyzePortfolio(request: PortfolioAnalysisRequest): Promise<PortfolioAnalysis> {
    ...
    const diversificationScore = this.calculateDiversificationScore(enrichedHoldings);
    ...
    const recommendations = includeRecommendations 
      ? await this.generateRecommendations(enrichedHoldings, riskMetrics, diversificationScore)
      : [];
    
    // Calculate performance metrics
    const performanceMetrics = await this.calculatePerformanceMetrics(enrichedHoldings);
   ...
   */
  private async enrichHoldingsWithMarketData(holdings: PortfolioHolding[]): Promise<PortfolioHolding[]> {
  ...
  private async calculateRiskMetrics(holdings: PortfolioHolding[]): Promise<{
    ...
    for (const holding of holdings) {
      try {
        const overviewResponse = await marketDataService.getCompanyOverview(holding.symbol);
    ...
    const portfolioVolatility = totalWeight > 0 ? avgVolatility / totalWeight : 0.15;
    
    // Simplified Sharpe ratio (assuming 2% risk-free rate)
    const riskFreeRate = 0.02;
    const expectedReturn = 0.10; // 10% expected return
   ...
   */
  private async analyzeSectorAllocation(holdings: PortfolioHolding[]): Promise<{ [key: string]: number }> {
    ...
    for (const holding of holdings) {
      try {
        ...
        sectors['Unknown'] = (sectors['Unknown'] || 0) + ((holding.marketValue || 0) / totalValue) * 100;
      }
    }
  ...
  private async generateRecommendations(
    ...
    if (diversificationScore < 50) {
      recommendations.push("🎯 Considere diversificar mais seu portfólio. Concentração alta pode aumentar o risco.");
    }
    
    if (holdings.length < 5) {
      recommendations.push("📈 Portfólio pequeno. Considere adicionar mais ativos para melhor diversificação.");
    }
    
    // Risk recommendations
    if (riskMetrics.volatility > 0.25) {
      recommendations.push("⚠️ Portfólio com alta volatilidade. Considere incluir ativos mais defensivos.");
    }
    
    if (riskMetrics.beta > 1.5) {
      recommendations.push("📊 Beta alto indica maior sensibilidade ao mercado. Avalie seu perfil de risco.");
    }
    
    if (riskMetrics.sharpeRatio < 1.0) {
      recommendations.push("💡 Sharpe ratio baixo. Busque ativos com melhor relação risco-retorno.");
    }
    ...
    if (gainers.length > 0) {
      recommendations.push(`🚀 Você tem ${gainers.length} posições com ganhos acima de 10%. Considere realizar lucros.`);
    }
    
    if (losers.length > 0) {
      recommendations.push(`⚡ Você tem ${losers.length} posições com perdas acima de 10%. Revise sua estratégia.`);
    }
  ...
  private async calculatePerformanceMetrics(
    ...
    return {
      ...
      oneMonthReturn: totalReturn * 0.20, // 20% of total return
      threeMonthReturn: totalReturn * 0.60, // 60% of total return
  ...
  generateAnalysisSummary(analysis: PortfolioAnalysis): string {
    ...
    const riskLevel = analysis.riskScore < 30 ? 'baixo' : analysis.riskScore < 70 ? 'moderado' : 'alto';
    const diversificationLevel = analysis.diversificationScore < 50 ? 'baixa' : analysis.diversificationScore < 80 ? 'moderada' : 'alta';
...
**🎯 Holdings (Top 5):**
${analysis.holdings.slice(0, 5).map(h => 
