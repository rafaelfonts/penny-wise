...
import { Bell, TrendingUp, TrendingDown, BarChart3, Clock, Edit, Trash2 } from 'lucide-react'
...
export function AlertCard({ alert, onToggle, onDelete }: AlertCardProps) {
  ...
  return (
    <Card className={`transition-all duration-200 hover:shadow-md ${
      alert.is_active ? 'border-l-4 border-l-blue-500' : 'border-l-4 border-l-gray-300'
    }`}>
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className={`p-2 rounded-lg ${
              alert.is_active ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-400'
            }`}>
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600 mt-1">
                Notify when price is <span className="font-medium">{getConditionText()}</span>
              </p>
            ...
            {alert.alert_type === 'price' && (
              <div className="mt-1">
                  ...
                  <div 
                    className={`h-2 rounded-full transition-all duration-300 ${
                      alert.condition_type === 'above' 
                        ? (alert.current_value >= alert.target_value ? 'bg-red-500' : 'bg-blue-500')
                        : (alert.current_value <= alert.target_value ? 'bg-red-500' : 'bg-blue-500')
                    }`}
                    style={{
                ...
                <div className="flex justify-between text-xs text-gray-500 mt-1">
                  ...
                  <span>
                    {((alert.current_value / alert.target_value - 1) * 100).toFixed(1)}%
                  </span>
                </div>
              </div>
            )}
