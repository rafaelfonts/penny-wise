...
export function PriceChart({
  ...
  const chartData = useMemo(() => {
    ...
    const points = sortedData.map((item, index) => {
      ...
      const y = padding + ((maxPrice - item.close) / priceRange) * (chartHeight - 2 * padding)
      return { x, y, price: item.close, timestamp: item.timestamp, volume: item.volume }
    })
  ...
  if (!chartData) {
    return (
      <Card className="w-full">
        ...
        <CardContent>
          <div 
            className="flex items-center justify-center bg-gray-50 rounded border-2 border-dashed border-gray-200"
          ...
          <div className="flex items-center gap-2">
            <div className="text-right">
              ...
              <div className={cn(
                ...
                {isPositive ? '+' : ''}{chartData.change.toFixed(2)} ({chartData.changePercent.toFixed(2)}%)
              </div>
            </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div className="text-center">
            ...
            <div className="font-medium">{formatPrice(chartData.firstPrice)}</div>
          </div>
          <div className="text-center">
            <div className="text-muted-foreground">Máxima</div>
            <div className="font-medium text-green-600">{formatPrice(chartData.maxPrice)}</div>
          </div>
          <div className="text-center">
            <div className="text-muted-foreground">Mínima</div>
            <div className="font-medium text-red-600">{formatPrice(chartData.minPrice)}</div>
          </div>
        </div>
        ...
        {chartData && chartData.totalDataPoints > 0 && (
          <div className="text-xs text-muted-foreground text-center">
            Volume médio: {(chartData.points.reduce((sum, p) => sum + p.volume, 0) / chartData.points.length).toLocaleString()} ações
          </div>
        )}
      </CardContent>
    </Card>
