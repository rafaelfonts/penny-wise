...
import { StockQuote, CompanyOverview, TechnicalIndicator } from '@/lib/types/market'
...
export function AnalysisCard({
  ...
  const generateAnalysisSignal = (): AnalysisSignal => {
    ...
    // Volume analysis
    if (quote.volume > 1000000) { // Simplified volume check without averageVolume
      buyScore += 15
    ...
    if (netScore > 30) {
      return { type: 'buy', strength: Math.min(80 + netScore - 30, 100), reason: reasons.join(', ') }
    } else if (netScore < -30) {
      return { type: 'sell', strength: Math.min(80 + Math.abs(netScore) - 30, 100), reason: reasons.join(', ') }
    } else {
      return { type: 'hold', strength: 50 + Math.abs(netScore), reason: reasons.join(', ') || 'Sinais mistos' }
    }
  }
  ...
  const getRiskLevel = () => {
    if (!quote) return { level: 'medium', text: 'Médio', color: 'bg-yellow-500' }
    
    const volatility = Math.abs(quote.changePercent)
    if (volatility > 5) return { level: 'high', text: 'Alto', color: 'bg-red-500' }
    if (volatility < 2) return { level: 'low', text: 'Baixo', color: 'bg-green-500' }
    ...
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          {onViewDetails && (
            <Button variant="outline" size="sm" onClick={() => onViewDetails(symbol)}>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Badge 
              variant="outline" 
              className={cn("flex items-center gap-1", getSignalColor(signal.type))}
            >
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className={cn(
                      ...
                      technicals.rsi.data[0].value > 70 ? "bg-red-500" :
                      technicals.rsi.data[0].value < 30 ? "bg-green-500" : "bg-yellow-500"
                    )} />
                  </div>
                </div>
              )}
              ...
              {technicals.macd && technicals.macd.data.length > 0 && (
                <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className={cn(
                      "w-2 h-2 rounded-full",
                      technicals.macd.data[0].value > 0 ? "bg-green-500" : "bg-red-500"
                    )} />
                  </div>
                </div>
              )}
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              {overview.peRatio && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{overview.peRatio.toFixed(1)}</span>
                </div>
              )}
              {overview.returnOnEquityTTM && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">ROE:</span>
                  <span className="font-medium">{(overview.returnOnEquityTTM * 100).toFixed(1)}%</span>
                </div>
              )}
              {overview.dividendYield && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Dividend:</span>
                  <span className="font-medium">{(overview.dividendYield * 100).toFixed(2)}%</span>
                </div>
              )}
              {quote.marketCap && (
                <div className="flex justify-between">
        ...
        <div className="text-xs text-muted-foreground pt-2 border-t">
          Análise atualizada em {new Date(quote.timestamp).toLocaleString('pt-BR')}
        </div>
      </CardContent>
    </Card>
  )
